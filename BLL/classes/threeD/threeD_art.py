...
class ThreeDArtGenerator:
            ...
            try:
                ...
                if shape == '1':
                    ...
                    if size < 3:
                        ...
                        horizontal_shift = int(input("Enter horizontal shift: "))
                        vertical_shift = int(input("Enter vertical shift: "))
                        self.art = Cube(size, horizontal_shift, vertical_shift)
                        break
                else:
    ...
    def display_ascii_art(self):
        if self.art:
            ...
            if projection:
                ascii_art = colored(projection, self.color)
                print("2D representation of the shape:")
                print(ascii_art)

    def get_art(self) -> str:
        art = self.art.get_three_d_art() if self.art else ""
        return colored(art, self.color)
